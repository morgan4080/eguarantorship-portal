...
const loanProductForm = reactive<LoanProductFormType>({
  name: '',
  interestRate: '',
  requiredGuarantors: '',
  maxPeriod: '',
...
const loanProductDetails = computed(() => {
  if (loanProductStore.getLoanProduct && loanProductStore.getLoanProduct.details) {
    loanProductForm.details = loanProductStore.getLoanProduct.details
  }
                ...
                <div class="flex flex-1 items-stretch overflow-hidden">
                  <main class="flex-1 overflow-y-auto">
                    <section aria-labelledby="primary-heading" class="flex h-full min-w-0 flex-1 flex-col lg:order-last">
                      ...
                      <dl>
                        <div class="grid grid-cols-6 gap-6 px-1.5">
                                  ...
                                  <section aria-labelledby="primary-heading" class="flex h-full min-w-0 flex-1 flex-col lg:order-last">
                                        ...
                                        <div v-for="(key, i) in loanProductDetails" :key="key+i" class="py-2 sm:grid sm:grid-cols-5 sm:gap-4 sm:py-3 sm:px-6">
                                          ...
                                          <dd class="mt-1 text-sm text-gray-900 sm:mt-0 col-span-2">
                                            <div class="border-b border-gray-300 focus-within:border-eg-bgopacity mt-1 sm:mt-0">
                                              <input :disabled="!otherDetailsEdit" required v-model="loanProductForm.details[key].value" type="text" class="block w-full border-0 border-b border-transparent bg-gray-50 focus:ring-eg-lightblue focus:border-eg-bgopacity focus:ring-0 sm:text-sm" placeholder="value" />
                                            </div>
                                          </dd>
                                    ...
                                    <div v-if="otherDetailsEdit" class="sm:divide-y sm:divide-gray-200 py-2 sm:py-3 sm:px-6">
                                      <div v-if="addingKey" class="flex rounded-md shadow-sm max-w-xs">
                                        <button :disabled="keyString === ''" @click="loanProductForm.details[`${keyString}`] = { value: '', type: 'TEXT' };addingKey = !addingKey" type="button" class="inline-flex items-center rounded-l-md border border-r-0 border-gray-300 bg-green-50 px-3 text-gray-500 sm:text-sm hover:bg-green-100 focus:bg-green-200">
